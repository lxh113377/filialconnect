...
### Changed
- **教程卡片网格改为生成产物**（第 33 轮，纯工具改动不制造版本号）。此前"加一篇教程"仍需在
  `pages/tutorials.html` 手写一张卡片，是结转五轮的可扩展性缺口。理由澄清：卡片里唯一不可推导的是
  **那张手绘插画**（每篇一张），其余（分类、标签、标题、简介、链接、顺序注释）全部可推导——
  于是把不可推导的两样落成数据：`content/tutorial-cards.json`（顺序 + 注释标签）与
  `content/card-art/<slug>.svg`（插画），其余进 `CARD_TMPL`。
  **验收＝逐字节复现**：规则化渲染器对现有 6 张卡 **6/6 字节全等**，切换生成后
  `pages/tutorials.html` 的 diff 只有 `BEGIN/END:TUTORIAL-CARDS` 两行标记，页面内容零改动。
- 缺图即拒绝：`content/card-art/<slug>.svg` 不存在时构建点名退出，不给"静默产出无图卡片"的机会
  （插画仍是人的内容决策，但忘了它不再是可漏的静默态）。
- 判据 `t_content_roster` 加四条：卡片区必须在生成器所属标记内；出厂卡片字节必须等于渲染结果
  （区内的手改活不过下一次构建）；插画集合必须与卡片集合互为全等（既不许缺、也不许留僵尸画）；
  卡片顺序只在 `content/tutorial-cards.json` 声明一处。
- 标签规则固化：`tut.<slug>.tag1` 必有、`tag2` 可选且自动带 accent 样式——原先"两标签卡"与
...
- **卡片文案改为派生**（第 34 轮，扩展性再进一格）：`tut.<slug>.h3|p|tagN` 不再是手写字典键。
  实测 `h3` 与已翻译的 `<h1>` **6/6 全等**，所以只有简介与标签是需要人写的文案，
  它们移进 `content/tutorials.json`（`card_p` / `card_tags`），`h3` 直接从 `h1` 派生。
  **验收＝出厂字典逐字节不变**：迁移后 `git diff assets/locales/` 为空，且 `pages/*.html` 零改动
  ⇒ 这次搬家对读者完全不可见。判据三条把它钉住：字典卡片文案必须等于内容、卡片键必须被归类为派生
  （否则改名/删篇会留下不 pruning 的僵尸键）、每个教程必须有双语卡片文案。
- **`last-updated` 的哈希口径只看页面可见字段**（`PAGE_VISIBLE_FIELDS`）。旧口径对整个内容对象做哈希，
  于是"给卡片网格加两个字段"会把**六篇教程全部刷成今天更新**——面向的是"这篇指南本文动没动"的声明，
  却把网格元数据的改动算作内容更新；在一个卖点是"可信"的适老站上这是假新鲜度。
  新增 `--rehash`：**只换哈希口径、绝不动日期**（口径迁移是工具变更，不是内容变更），
  本轮实测 `6 hashes moved / 0 dates moved`；判据双向守：加网格字段或改 `slug` 不得移动戳，
  改一步一步正文**必须**移动（防口径窄成常数）。
- `extract` 不再静默丢弃只存在于内容里的作者字段（`illustration`/`img_alt`/`category`/`card_p`/`card_tags`）：
  按 slug 从上一次的 JSON 结转，缺哪个就点名拒绝——原先它重建记录时只搬 HTML 里能看见的东西。
- 字典写入改为 `sort_keys=True`：键序不再取决于"谁在什么时候手工插在哪一行"，
  加内容时 diff 位置确定（本轮之前每次插键都要自己算二分位置）。
- `CONTRIBUTING.md`：新增教程第 4 步不再写字典卡片键，卡片文案随内容文件走。
...
- **判据在格式化自己那条失败证据时崩掉**（第 36 轮变异电池 M4 抓出，缺陷早在第 35 轮）。
  `check()` 用字符串拼接把 detail 附到判据名上，而 4 处调用点传的是**列表**
  （`out.strip().splitlines()[-1:]`、`tail[:1]`）。判据绿的时候完全看不出来；一旦它变红，
  那行拼接就抛 `TypeError: can only concatenate str (not "list") to str`，第 31 轮加的 abort 护栏
  于是把这次运行记成"判据崩了"而不是"判据抓到了变异"——**恰好在它抓住缺陷的那一刻销毁了证据**。
  现在 `check()` 无条件把 detail 转成字符串（失败路径不许再有能力失败），4 个调用点改走新的
  `last_line()`，并且 M4 复跑为 HIT。这是"一个变红的判据毁掉全链证据"那一类的第二个实例。
