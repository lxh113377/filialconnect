...
- **基线本身入门禁 `t_perf_measurement`**（第二十五轮下半，v1.6.0 之后）。上一轮的教训是"分数能过而
  ...
  **每页的最坏样本也要过预算**（不是中位数）、unstable 页必须在 summary 里点名、
  ...
  9 条变异逐条实测：改 runs=1 / 删 server / 删一页 / 删一条 LCP 极差 / 删 unstable 键 /
  最坏样本调成 0.4 / 删 worst_sample_breaches / summary 少点一个名 / 摘掉预热优先级——各自转红，
  未变异对照与还原后 0 红。
- **`perf-probe` 把"最坏样本"提升为一等公民**：summary 新增 `worst_sample_breaches`；
  ...
  基线已用 `--runs=3` 全量重测（18 URL × 3 = 54 次 Lighthouse），桌面最低中位 0.94、
  移动 4 页全部 0.95，`worst_sample_breaches` 两侧均为空。
...
### Fixed
- **桌面 fraud-database 仍慢 1.16 s（记录，不当已修）**：预热改优先级后双峰消失、
  但 3 次采样的桌面 LCP 稳定在 1684-1685 ms，而其余 13 页 483-524 ms，差值与 1,488 KiB
  名单在桌面模拟带宽下的下载时间吻合。本机 `python -m http.server` 不压缩（生产 gzip ≈532 KiB），
  绝对值放大但机制真实。彻底解法是名单分片，另立任务，不混进本轮发版。
