...
- 无外部请求：字体为系统栈（Noto Serif SC / PingFang SC 等），离线能力覆盖到**页面自身引用的
  每一个同源样式与脚本**——预缓存 14 页 + `manifest.json`，其余样式/脚本/图片走
  `StaleWhileRevalidate` / `CacheFirst` 路由（v1.6.1 之前脚本与样式**一条离线路由都没有**，
  装了再离线打开就是没样式的空壳）。覆盖面由 `node tools/build.mjs check` 逐文件回问
  "这个引用离线拿得到吗"，不是靠这句话
...
- 可疑链接自查用的离线名单为**国际钓鱼域名**（83,097 条，gzip 后 532 KB），不含境内域名、
  ...
  下载后面）——v1.6.1 之前是"页面加载即下载整份名单"，那会把该页桌面 LCP 从 523 ms 拖到
  1,685 ms（间歇 9.75 s）。已评估并按实测数据**否决**了
  按 TLD 分片方案（.com 仅占 43.4%、共 527 个 TLD，分片最多多省一半且仅在查 .com 时生效）。
